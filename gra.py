...
for i in range(0, ilosc):
    picked = int(input("Gracz " + imiona[0] + " wybiera: (1- kamień, 2- papier, 3- nożyce\n"))
    if (tryb == 2):
        picked2 = int(input("Gracz " + imiona[1] + " wybiera: (1- kamień, 2- papier, 3- nożyce\n"))
    else:
        picked2 = random.randrange(1, 4)
        print("Komputer wybiera " + opcje[(picked2-1)])
    print(picked)
    print(picked2)
    if picked == picked2:
        print("Remis!")
        ...
        if picked == 1:
            if picked2 == 3:
                print("Wygral " + imiona[0])
                w1 += 1
            else:
                print("Wygral " + imiona[1])
                w2 += 1
        else:
            if picked == 2:
                if picked2 == 1:
